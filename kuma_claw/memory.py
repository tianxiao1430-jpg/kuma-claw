...
class MemoryStore:
    ...
    def add(self, entry: MemoryEntry):
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (entry.id, entry.content, entry.source, json.dumps(entry.metadata),
              entry.created_at, entry.updated_at, embedding_blob))
        conn.commit()
    ...
    def search_fts(self, query: str, limit: int = 10) -> List[MemorySearchResult]:
        ...
        return [MemorySearchResult(entry=self._row_to_entry(r), score=abs(r["score"])) 
                for r in rows]
